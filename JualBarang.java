...
import static marketplace.DatabaseConnection.getNamaPembeli;
...
public class JualBarang extends javax.swing.JPanel {
    ...
    private String idKasir; // Ini harus diisi dari MainMarketplace
    private String namaPembeliFromField; // Tambahkan ini untuk menyimpan nama pembeli
    ...
    public JualBarang(MainMarketplace main, String nama) {
        ...
        initComponents();
        setupTable();
        // Inisialisasi idKasir saat JualBarang dibuat atau saat halaman ditampilkan
        this.idKasir = main.getCurrentKasirId(); // Ambil dari MainMarketplace
    }

    private void setupTable() {
        tableModel = new DefaultTableModel(
            new String[]{"Nomor", "Nama Barang", "Jumlah Barang", "Harga Satuan", "Total Harga"}, 0 // Hapus "Metode Pembayaran" dari sini, karena itu per transaksi, bukan per item
        ) {
            @Override
    ...
    private void updateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateButtonActionPerformed
        ...
        if (selectedRow != -1) {
            try {
                ...
                DatabaseConnection.Produk produk = DatabaseConnection.getProduk(idBarang);
                ...
                // metode pembayaran tidak di tabel per item lagi
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        try {
            ...
            String namaPembeliInput = jTextField3.getText().trim(); // Ambil nama pembeli dari field
            String idBarang = jTextField2.getText().trim();
            String jumlahStr = jTextField4.getText().trim();
            String metodePembayaran = jComboBox1.getSelectedItem().toString();

            // Validasi input yang lebih lengkap
            if (idPembeli.isEmpty() || namaPembeliInput.isEmpty() || idBarang.isEmpty() || jumlahStr.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Mohon isi semua field!", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
            this.namaPembeliFromField = namaPembeliInput; // Simpan nama pembeli untuk digunakan saat cetak struk
            ...
            item.setHargaSatuan(produk.getHarga());
            item.setTotalHarga(jumlah * produk.getHarga());
            item.setMetodePembayaran(metodePembayaran); // Ini untuk keperluan struck, bukan disimpan per item di DB
            ...
            // Clear fields barang saja, ID Pembeli dan Nama Pembeli tetap
            jTextField2.setText("");
            jTextField4.setText("");
    ...
    private void buatStruckActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buatStruckActionPerformed
        ...
        String metodePembayaran = jComboBox1.getSelectedItem().toString(); // Ambil metode pembayaran terakhir yang dipilih

        float totalHargaTransaksi = 0;
        for (ItemTransaksi item : listBarang) {
            totalHargaTransaksi += item.getTotalHarga();
        }

        // Kembalian akan dihitung di panel Struck
        float kembalianTransaksi = 0;

        try {
            // Panggil simpanTransaksiMultiple satu kali untuk seluruh listBarang
            boolean success = DatabaseConnection.simpanTransaksiMultiple(listBarang, idPembeli, idKasir, totalHargaTransaksi, kembalianTransaksi, metodePembayaran);

            if (success) {
                JOptionPane.showMessageDialog(this, "Transaksi berhasil disimpan!");

                // Pass data ke struck
                main.setTransactionData(listBarang, idPembeli, idKasir);
                
                // Set nama pembeli di MainMarketplace agar Struck bisa mengaksesnya
                main.setCustomerNameForStruck(namaPembeliFromField);

                // Bersihkan listBarang dan tabel setelah berhasil disimpan
                listBarang.clear();
                tableModel.setRowCount(0);
                clearFields(); // Bersihkan field input saja
                jTextField1.setText(""); // Bersihkan ID Pembeli
                jTextField3.setText(""); // Bersihkan Nama Pembeli
    ...
    public void testDatabaseConnection() {
        try {
            ...
            DatabaseConnection.Produk produk = DatabaseConnection.getProduk(testId);
    ...
    // Kelas ItemTransaksi tetap sama
    class ItemTransaksi {
        ...
        private String metodePembayaran; // Tetap simpan di sini untuk dicetak di struck
